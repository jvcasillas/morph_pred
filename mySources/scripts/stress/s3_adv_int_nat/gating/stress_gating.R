...
# actually conducted
stress_gating_ttest$Group <- factor(stress_gating_ttest$Group, levels = c("LA", "IN", "S"))
...
ggplot(stress_gating_ttest, aes(x = Group, y = estimate, color = Condition, 
    ...
    ylim(0.3, 1.0) +
    ggtitle('Mean accuracy and lower-bound 99% confidence interval') + 
    ...
    labs(title = 'Response accuracy as a function of stress.', 
         y = '% correct', x = '', caption = 'Mean and lower-bound 99% CI') + 
...
stress_gating_mod$Group <- factor(stress_gating_mod$Group, levels = c("S", "LA",  "IN"))
...
anova(gating_mod_0, gating_mod_group, test = "Chisq")
anova(gating_mod_group, gating_mod_cond, test = "Chisq")
anova(gating_mod_cond, gating_mod_coda, test = "Chisq")
anova(gating_mod_coda, gating_mod_wm, test = "Chisq")
anova(gating_mod_wm, gating_mod_int1, test = "Chisq")
anova(gating_mod_wm, gating_mod_int2, test = "Chisq")


#    AIC    BIC  logLik deviance   Chisq Chi Df Pr(>Chisq)
# 2109.0 2175.9 -1042.5   2085.0                           gating_mod_0        
# 2136.0 2214.6 -1054.0   2108.0  6.4624      2    0.03951 gating_mod_group ***
# 2137.9 2222.0 -1053.9   2107.9  0.1638      1     0.6857 gating_mod_cond     
# 2139.3 2229.1 -1053.7   2107.3  0.5113      1     0.4746 gating_mod_coda     
# 2136.1 2231.5 -1051.1   2102.1  5.2227      1    0.02229 gating_mod_wm    *  
# 2137.3 2243.8 -1049.6   2099.3  2.8622      2     0.2391 gating_mod_int1     
# 2136.9 2237.9 -1050.5   2100.9  1.1915      1      0.275 gating_mod_int2     
...
# (Intercept)  1.45561    0.15046   9.674   <2e-16 ***
# GroupLA      0.00619    0.17972   0.034   0.9725    
# GroupIN     -0.55974    0.21896  -2.556   0.0106 *  
# wmScaled     0.18099    0.07666   2.361   0.0182 *  
