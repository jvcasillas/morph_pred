...
The time course data from the eye-tracking task were analized using growth 
...
b0_coda <- params$B[params$subscript == "31"] %>% as.numeric
b0_increase <- inv_logit(b0 + b0_coda) - inv_logit(b0)
...
target_offset_prob <- function(group, coda, cond) {
  ...
  output <- paste0("Probability = ", vals$prob, "; LB = ", vals$prob_lb, "; UB = ", vals$prob_ub)
  return(output)
}
...
Figure X plots the model estimates from the GCA. 
The full model summary is available in Appendix X. 
...
Table X provides estimates ±SE for all groups in all conditions. 
...
of flater inflection points for CVC oxytone targets. 
Both learner groups showed later target fixations in the default, CV 
paroxytone condition. 
This assertion is corrorborated by examening the NIN and IN's proportion of 
target fixations at the target syllable offset (see Table XXX).
Specifically, the model estimates show that NIN did not anticipate with CV 
...
Thus the learner groups differed from each other in that 
