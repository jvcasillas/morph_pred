...
The time course data from the eye-tracking task were analyzed using growth 
curve analysis (GCA, Mirman, 2016). 
...
Figure 1 plots the model estimates from the GCA. 
The full model summary is available in Appendices 1 and 2. 
...
The was a main effect of lexical stress on the quadratic time term 
(`r pretty_chi_result(nested_model_comparisons$ss_cond_anova, 
"gca_mod_ss_cond_2")`). 
Averaging over syllable structure, a change from paroxytonic to oxytonic 
stress decreased the bowing of the trajectory at the center of the time 
course (`r report_row("22")`) such that M fixated on oxytonic targets earlier 
than paroxytonic targets. 
...
"gca_mod_ss_coda_3")`), as well as syllable structure &times; lexical stress 
interaction on the linear time term  
...
overall slope (`r report_row("31")`) and the bowing of the inflections points 
of closed, paroxytonic syllables (`r report_row("15")`).
...
processing. 
Specifically, defaulting from a paroxytone with a CV penult, one observes 
earlier target fixations with the addition of a coda and with a shift of 
stress to the final syllable, suggesting that marked sequences facilitate 
lexical access in native speakers. 


**+++Table 1 here +++**  


With regard to the learners, there was a simple interaction of the quadratic 
...
syllable, indicating later overall fixations on the target. 
Additionally, there was a lexical stress &times; syllable structure 
interaction on the linear slope (`r report_row("23")`), such that NIN had a 
steeper slope than M in CV syllables of paroxtone words. 
This indicates that the NIN fixated on targets later the default condition, 
but earlier in other conditions. 
...
of sharper inflection points for CV oxytone targets. 


**+++Figure 2 here+++**  


Both learner groups showed later target fixations in the default, CV 
...
Pairwise comparisons (see Appendix 3) showed that the learner groups also 
...
Upon inspecting Figure 2 one can observe that the learners have nearly 
identical trajectories for CV paroxytones. 
In all other conditions IN have steeper slopes with more bowed inflection 
points, indicating later target fixations with regard to the NIN. 
That said, in all conditions the IN group fixate on targets in equal proportion 
to NIN at the offset of the target syllable (the dotted vertical lines), 
suggesting IN fixate on targets later but at a faster rate. 

There was no effect of age on the intercept (`r 
pretty_chi_result(nested_model_comparisons$int_age_anova, 
"gca_mod_int_age_0")`), nor on any of the orthogonal polynomial time terms 
(Time^1^ &times; Age: `r 
pretty_chi_result(nested_model_comparisons$int_age_anova, 
"gca_mod_int_age_1")`; Time^2^ &times; Age: `r 
pretty_chi_result(nested_model_comparisons$int_age_anova, 
"gca_mod_int_age_2")`; Time^3^ &times; Age: `r 
pretty_chi_result(nested_model_comparisons$int_age_anova, 
"gca_mod_int_age_3")`)
